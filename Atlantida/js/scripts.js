...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const showMessage = (element, text, type) => { if (!element) return; element.textContent = text; element.className = `message ${type}`; };
  const clearMessage = element => { if (!element) return; element.textContent = ''; element.className = 'message'; };
  ...
  document.querySelectorAll('form[data-form]').forEach(form => {
    ...
    form.addEventListener('submit', event => {
      ...
      if (!nameValue || !emailValue || !isValidEmail(emailValue) || !textValue || textValue.length < 10 || !serviceValue) {
        showMessage(msg, 'Revisa los campos obligatorios. El correo debe ser válido y el mensaje debe tener al menos 10 caracteres.', 'error');
        return;
      }
